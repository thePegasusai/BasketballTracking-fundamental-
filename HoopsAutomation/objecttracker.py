...
from yolo3.utils import Load_yolo_model, image_preprocess, postprocess_boxes, nms, draw_bbox, read_class_names
...
def Object_tracking(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_thresehold=0.3, iou_threshold=0.45, rectangle_colors='', Track_only = [], custom_yolo=None, custom_classes=YOLO_CUSTOM_CLASSES, Custom_track_only=[]):
    # Defintion of the paramets
    ...
    if video_path:
        vid = cv2.VideoCapture(video_path) # detect on video_path
    else:
    ...
    # set a bunch of flags and variables for made baskets and posessions 
    posession = None
        ...
        if custom_yolo:
            ...
            for bbox in custom_bboxes:
                if len(Custom_track_only) !=0 and NUM_CUSTOM_CLASS[int(bbox[5])] in Custom_track_only or len(Custom_track_only) == 0:
                     ...
                     made_baseket_box = None
                     for i, bbox in enumerate(custom_bboxes):
                         # loop through each bounding box to get the "best one" of the frame
                         # we do this becasue sometimes our model will detect two, and we know there can only be one
                        name = custom_names[i]
                    ...
                    # if it sees a basketball, put a box on it and note the center (for possesion)
                    if basketball_box is not None:
                        ...
                        cv2.rectangle(original_frame, "basketball" + "-" + str(highest_scoring_basketball),(int(basketball_box[0]), int(basketball_box[1]-10)),0, 0.5, (255,255,255),1)
                        basketball_center = ( (basketball_box[2]+basketball_box[0])/2, (basketball_box[1])/2 )
                        
                    if made_basketball_box is not None:
                        ...
                        made_basket_first_frame = += 1
                        
                        # if there were 3 consecutive frames havent marked the basket yet then lets count it !
                    ...
                    patches = [gdet.extract_iamge_patch(frame, box, [box[3], box[2]]) for box in boxes]
                    ...
                    for track in tracker.tracks:
                        ...
                        # if there is a basketball in the frame, and its "in" a persons bounding box, check what box it is in for posession
                        if basket_center:
                            if basketball_center[0] >= bbox[0] and basketball_center[0] <= bbox[2]:
                                if basketball_center[1] >= bbox[1] and basketball_center[1] <= bbox[3]:
                                    ...
                                    if color_ratio <= .2:
                                        # light team
                                        posession_list.append(0)
                                    else:
                                        # dark team
                                        possesion_list.append(1)
                        
                        else:
                            # no basketball in frame
                            # posession_list.append(-1)
                            # test_list.pop(0)
                            pass
                    ...
                    if check_possession:
                        if len(possession_list) > 60:
                            ...
                            # it weights the most recent detectiosn more
                            # this algo is a WIP
                            possession_list = possession_list[-60:]
                            # full_av = sum(possession_list)/len(possession)
                            ...
                            # most_common_possesson = stats.mode(possession_list)[0]
